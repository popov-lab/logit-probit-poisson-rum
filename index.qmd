...
abstract: |
  This study embeds the two canonical discrete choice specifications - multinomial logit and multinomial probit - in a single generative family built on a count process. The model uses a Poisson count process race, wherein $K$ alternatives generate events according to independent Poisson processes. A choice is determined when an alternative reaches a cumulative count threshold $\theta$. At the single-event threshold, $\theta=1$, the model yields the Multinomial Logit (Luce choice rule). By normalizing the utility noise to maintain a constant variance, I show that as $\theta \to \infty$, the model converges to the independent Multinomial Probit. The distributional ingredients of this construction are classical - the induced errors form the log-Gamma family long studied in survival analysis, and Poisson race models of static choice have a history in psychometrics. The contribution is the synthesis: a variance-standardized formulation in which logit and probit occupy the two endpoints of one generative family, with $\theta$ governing the shape of the error distribution and a separate parameter, $\beta$, modulating the variance, while the systematic utilities $v_i$ are shared across all regimes. The framework further yields the correlated probit through shared evidence streams, implies an upper bound coupling attainable error correlations to utility differences and the threshold, and provides a process-level account of why signal-detection parameters, unlike softmax parameters, remain invariant across set size. The unification is distributional rather than dynamical: changes in $\theta$ do not represent a single decision-making system adjusting its threshold in real time, but rather compare different accumulation regimes at matched discriminability.
...
3.  **A bound coupling correlation, utilities, and threshold.** Extending the race with shared feature generators yields the correlated Multinomial Probit in the limit, and the generative construction implies a strict upper bound, $\rho_{\max} = \exp(-(v_1 - v_2)\sigma_\theta/2)$ (where $\sigma_\theta$ is the scale of the unstandardized log-Gamma noise), on the error correlation attainable between alternatives of unequal utility at a given threshold (@sec-correlated-errors).
...
Result 1 involves no new distribution theory. The noise term $-\log G_i$ is a member of the log-gamma family whose properties were characterized by @consulLoggammaDistributionIts1971a and which @prenticeLogGammaModel1974 developed, building on the generalized gamma distribution of @stacyGeneralizationGamma1962, into a standard tool of survival analysis [@lawlessInferenceGeneralizedGamma1980]. In Prentice's parameterization the shape parameter plays exactly the interpolating role that $\theta$ plays here: the extreme value distributions are included as boundary cases and the normal arises as the central limit. What the Poisson race adds is an interpretation. Embedded in a race with a count threshold, the shape parameter of this classical family acquires a process meaning — the amount of evidence required before commitment — and the distributional family becomes a family of random utility models.
...
Distributionally, Result 3 restates a classical fact: the standardized log-gamma distribution converges to the standard normal as its shape parameter grows, which is precisely the sense in which the normal is the central case of Prentice's [-@prenticeLogGammaModel1974] log-gamma family. In the binary case, the resulting convergence of choice probabilities to the probit form likewise recovers the Thurstone–Mosteller endpoint of Stern's [-@sternContinuumPairedComparisons1990] continuum of paired comparison models. The content of Result 3 is the multinomial, choice-level statement: under the matched-discriminability identification of @sec-variance-normalization, the full vector of choice probabilities converges to that of the independent Multinomial Probit for any number of alternatives, so that the two canonical discrete choice models occupy the two endpoints of a single generative family.
...
To my knowledge, this bound has not been noted before. It distinguishes the generative construction from the descriptive use of a correlated probit, in which the correlations and the utilities are free parameters that can be combined arbitrarily: in a Poisson race realized by non-negative event rates, the attainable correlation is jointly constrained by the utility difference and the accumulation threshold, a coupling that is in principle testable.
...
From a mathematical standpoint, all components of the present framework are classical: exponential races yield Luce's choice rule, Gamma waiting times arise from accumulated Poisson events, and asymptotic normality follows from the Central Limit Theorem. More than that, both halves of the bridge have been built before, separately. As a family of distributions, the log-Gamma interpolation between extreme-value and normal shapes is Prentice's [-@prenticeLogGammaModel1974] log-gamma family, a standard object in survival analysis [@stacyGeneralizationGamma1962; @lawlessInferenceGeneralizedGamma1980]; and as a family of binary choice functions, the incomplete-Beta continuum from Bradley–Terry to Thurstone–Mosteller was described by @sternContinuumPairedComparisons1990. The contribution of the present work lies in assembling these elements into a single generative account of multinomial choice, identifying the variance standardization under which the limiting behavior remains non-degenerate, and drawing out its consequences: the multinomial probit endpoint, the $\rho_{\max}$ bound, and the parameter-invariance crossover.

The closest precedents on the process side come from the literature on Poisson race models of conjoint choice. @otterIntegratedModelDiscrete2008 built an integrated model of choice and response time on the same thresholded race and stated several of the links documented here, without pursuing the proofs or the interpolation: their model recovers the logit exactly at threshold one, produces structured IIA departures at higher thresholds (which identify the threshold from choices alone), and yields a probit if the race is instead stopped at a fixed exogenous deadline, where accumulated counts are approximately normal. @ruanDependentPoissonRace2008a developed the dependent Poisson race as an empirical competitor to the multinomial logit and demonstrated superior fit to conjoint choice data. These efforts are complementary to the present one but aim at a different target: the race serves as a richer descriptive model to be fit, with the threshold as a free parameter. Here the threshold is instead treated as an interpolation index, and the race is used to establish a theoretical relationship — that multinomial logit and independent multinomial probit are the two endpoints of one family under matched discriminability, with the interpolation characterized distributionally (log-Gamma noise), proved in the limit, and quantified by simulation. The probit endpoint in particular differs from the fixed-deadline construction of @otterIntegratedModelDiscrete2008: there the stopping rule changes, whereas here the independent probit arises within the threshold-stopping family itself, as its variance-standardized limit. The general correspondence between race mechanisms and random utility representations goes back further still, to the horse-race random utility model of @marleyHorseRaceRandom1992.
