...
Most directly, Ruan et al.
(\citeproc{ref-ruanDependentPoissonRace2008a}{2008}) developed Poisson
race models --- with both independent and dependent accumulators --- as
a process-based random-utility account of \emph{static} conjoint choice,
derived their choice probabilities, and showed that dependent Poisson
races outperform the multinomial logit empirically. Poisson races as
generators of static choice models are therefore not new in themselves.
...
Result 1 involves no new distribution theory. The noise term
\(-\log G_i\) is a member of the log-gamma family whose properties were
characterized by Consul and Jain
(\citeproc{ref-consulLoggammaDistributionIts1971a}{1971}) and which
Prentice (\citeproc{ref-prenticeLogGammaModel1974}{1974}) developed,
building on the generalized gamma distribution of Stacy
(\citeproc{ref-stacyGeneralizationGamma1962}{1962}), into a standard
tool of survival analysis
(\citeproc{ref-lawlessInferenceGeneralizedGamma1980}{Lawless, 1980}). In
Prentice's parameterization the shape parameter plays exactly the
interpolating role that \(\theta\) plays here: the extreme value
distributions are included as boundary cases and the normal arises as
the central limit. What the Poisson race adds is an interpretation.
Embedded in a race with a count threshold, the shape parameter of this
classical family acquires a process meaning --- the amount of evidence
required before commitment --- and the distributional family becomes a
...
Distributionally, Result 3 restates a classical fact: the standardized
log-gamma distribution converges to the standard normal as its shape
parameter grows, which is precisely the sense in which the normal is the
central case of Prentice's
(\citeproc{ref-prenticeLogGammaModel1974}{1974}) log-gamma family. In
the binary case, the resulting convergence of choice probabilities to
the probit form likewise recovers the Thurstone--Mosteller endpoint of
Stern's (\citeproc{ref-sternContinuumPairedComparisons1990}{1990})
continuum of paired comparison models. The content of Result 3 is the
multinomial, choice-level statement: under the matched-discriminability
identification of Section~\ref{sec-variance-normalization}, the full
vector of choice probabilities converges to that of the independent
Multinomial Probit for any number of alternatives, so that the two
canonical discrete choice models occupy the two endpoints of a single
generative family.
...
(\citeproc{ref-sternContinuumPairedComparisons1990}{1990}), who
identified the Bradley--Terry model at \(\theta = 1\) and the
Thurstone--Mosteller model in the large-\(\theta\) limit, and the same
...
follows from the Central Limit Theorem. More than that, both halves of
the bridge have been built before, separately. As a family of
distributions, the log-Gamma interpolation between extreme-value and
normal shapes is Prentice's
(\citeproc{ref-prenticeLogGammaModel1974}{1974}) log-gamma family, a
standard object in survival analysis
(\citeproc{ref-lawlessInferenceGeneralizedGamma1980}{Lawless, 1980};
\citeproc{ref-stacyGeneralizationGamma1962}{Stacy, 1962}); and as a
family of binary choice functions, the incomplete-Beta continuum from
Bradley--Terry to Thurstone--Mosteller was described by Stern
(\citeproc{ref-sternContinuumPairedComparisons1990}{1990}). The
...
The closest precedent on the process side is the dependent Poisson race
model of Ruan et al.
(\citeproc{ref-ruanDependentPoissonRace2008a}{2008}), which shares this
paper's starting point: Poisson races as a process-based random utility
account of static choice, with dependence introduced through shared
event streams. The two efforts are complementary but aim at different
targets. Ruan and colleagues positioned the dependent Poisson race as an
empirical competitor to the multinomial logit and demonstrated superior
fit to conjoint choice data, with the race parameters estimated
descriptively. Here, the threshold is instead treated as an
interpolation index, and the race is used to establish a theoretical
relationship --- that multinomial logit and independent multinomial
probit are the two endpoints of one family under matched
discriminability. To my knowledge, the probit side of this
correspondence has not previously been stated in the choice literature,
where the Poisson race has figured as an alternative to the logit rather
than as a bridge between logit and probit. The general correspondence
between race mechanisms and random utility representations goes back
further still, to the horse-race random utility model of Marley and
Colonius (\citeproc{ref-marleyHorseRaceRandom1992}{1992}).
