...
framework further yields the correlated probit through shared evidence
streams, implies an upper bound coupling attainable error correlations
to utility differences and the threshold, and provides a process-level
account of why signal-detection parameters, unlike softmax parameters,
remain invariant across set size. The unification is distributional
rather than dynamical: changes in \(\theta\) do not represent a single
decision-making system adjusting its threshold in real time, but rather
compare different accumulation regimes at matched discriminability. }
...
\item
  \textbf{A bound coupling correlation, utilities, and threshold.}
  Extending the race with shared feature generators yields the
  correlated Multinomial Probit in the limit, and the generative
  construction implies a strict upper bound,
  \(\rho_{\max} = \exp(-(v_1 - v_2)\sigma_\theta/2)\) (where
  \(\sigma_\theta\) is the scale of the unstandardized log-Gamma noise),
  on the error correlation attainable between alternatives of unequal
...
To my knowledge, this bound has not been noted before. It distinguishes
the generative construction from the descriptive use of a correlated
probit, in which the correlations and the utilities are free parameters
that can be combined arbitrarily: in a Poisson race realized by
non-negative event rates, the attainable correlation is jointly
constrained by the utility difference and the accumulation threshold, a
coupling that is in principle testable.
...
non-degenerate, and drawing out its consequences: the multinomial probit
endpoint, the \(\rho_{\max}\) bound, and the parameter-invariance
crossover.
